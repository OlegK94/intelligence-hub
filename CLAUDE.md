...
2. Read those pages (use `python3 tools/search.py "query"` if the wiki is large).
...
Naive full-text search over wiki pages (BM25-style). Hand results to the LLM for larger queries.

```bash
python3 tools/search.py "supplement stack"
python3 tools/search.py "Wagglz" --limit 10
```
